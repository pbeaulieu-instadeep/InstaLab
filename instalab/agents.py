"""This module defines a framework for a multi-agent system.

It is designed to simulate a research environment. It includes agents representing different roles
(PhD Student, Postdoc, Professor, Reviewers) and functionalities for tasks
such as literature review, experiment planning, report writing, results
interpretation, and peer review of research papers.

The system utilizes a language model (via `inference.query_model`) to generate
responses and perform tasks based on detailed prompts and agent roles.
"""

from __future__ import annotations

import json
import re
from typing import Any

from instalab.inference import query_model
from instalab.utils import extract_prompt


def extract_json_between_markers(llm_output: str) -> dict[str, Any] | None:
    """Extracts JSON content from a string, typically an LLM output.

    It first tries to find JSON enclosed in ```json ... ``` markers.
    If not found, it attempts a more general regex for JSON-like structures.
    It also tries to clean the JSON string by removing invalid control
    characters if the initial parsing fails.

    Args:
        llm_output: The string output from an LLM potentially containing JSON.

    Returns:
        A dictionary parsed from the JSON string if found and valid,
        otherwise None.
    """
    # Regular expression pattern to find JSON content between ```json and ```
    json_pattern = r"```json(.*?)```"
    matches = re.findall(json_pattern, llm_output, re.DOTALL)

    if not matches:
        # Fallback: Try to find any JSON-like content in the output
        # This pattern is very general and might match non-JSON,
        # but json.loads will verify.
        json_pattern_fallback = (
            r"\{(?:[^{}]|(?R))*\}"  # More robust than r"\{.*?\}" for nested structures
        )
        matches = re.findall(json_pattern_fallback, llm_output, re.DOTALL)

    for json_string in matches:
        json_string = json_string.strip()
        try:
            parsed_json = json.loads(json_string)
            return parsed_json
        except json.JSONDecodeError:
            # Attempt to fix common JSON issues
            try:
                # Remove invalid control characters (ASCII 0-31 and 127)
                json_string_clean = "".join(
                    char for char in json_string if ord(char) >= 32 and ord(char) != 127
                )
                # Another common issue is trailing commas, but json.loads usually handles them with more specific errors.
                # For this example, focusing on control characters.
                # A more robust cleaner might be needed for more complex errors.
                parsed_json = json.loads(json_string_clean)
                return parsed_json
            except json.JSONDecodeError:
                # If cleaning also fails, try the next match
                continue

    return None  # No valid JSON found


def get_score(
    outlined_plan: str,
    latex: str,
    reviewer_type: str | None = None,
    attempts: int = 3,
    gemini_api_key: str | None = None,
) -> tuple[float | None, str, bool]:
    """Evaluates a research paper (LaTeX) against an outlined plan using an LLM.

    The function prompts a language model (Gemini) to act as a reviewer,
    following a detailed template (NeurIPS review form). It extracts the review
    in JSON format, calculates a weighted performance score based on various
    criteria, and returns the score, the full textual review, and a success flag.

    Args:
        outlined_plan: A string describing the research plan.
        latex: A string containing the LaTeX content of the research paper.
        reviewer_type: An optional string to specify the reviewer's persona.
        attempts: The number of attempts to get a valid score from the LLM.
        gemini_api_key: The API key for the Gemini model.

    Returns:
        A tuple containing:
        - Optional[float]: The calculated performance score (0-10), or None on failure.
        - str: The full review text from the LLM, or an error message.
        - bool: True if scoring was successful, False otherwise.
    """
    last_exception_message = ""
    for _attempt in range(attempts):
        try:
            # template inherited from the AI Scientist (good work on this prompt Sakana AI team :D)
            template_instructions = """
            Respond in the following format:

            THOUGHT:
            <THOUGHT>

            REVIEW JSON:
            ```json
            <JSON>
            ```

            In <THOUGHT>, first briefly discuss your intuitions and reasoning for the evaluation.
            Detail your high-level arguments, necessary choices and desired outcomes of the review.
            Do not make generic comments here, but be specific to your current paper.
            Treat this as the note-taking phase of your review.

            In <JSON>, provide the review in JSON format with the following fields in the order:
            - "Summary": A summary of the paper content and its contributions.
            - "Strengths": A list of strengths of the paper.
            - "Weaknesses": A list of weaknesses of the paper.
            - "Originality": A rating from 1 to 4 (low, medium, high, very high).
            - "Quality": A rating from 1 to 4 (low, medium, high, very high).
            - "Clarity": A rating from 1 to 4 (low, medium, high, very high).
            - "Significance": A rating from 1 to 4 (low, medium, high, very high).
            - "Questions": A set of clarifying questions to be answered by the paper authors.
            - "Limitations": A set of limitations and potential negative societal impacts of the work.
            - "Ethical Concerns": A boolean value indicating whether there are ethical concerns.
            - "Soundness": A rating from 1 to 4 (poor, fair, good, excellent).
            - "Presentation": A rating from 1 to 4 (poor, fair, good, excellent).
            - "Contribution": A rating from 1 to 4 (poor, fair, good, excellent).
            - "Overall": A rating from 1 to 10 (very strong reject to award quality).
            - "Confidence": A rating from 1 to 5 (low, medium, high, very high, absolute).
            - "Decision": A decision that has to be one of the following: Accept, Reject.

            For the "Decision" field, don't use Weak Accept, Borderline Accept, Borderline Reject, or Strong Reject. Instead, only use Accept or Reject.
            This JSON will be automatically parsed, so ensure the format is precise.
            """
            neurips_form = (
                """
                ## Review Form
                Below is a description of the questions you will be asked on the review form for each paper and some guidelines on what to consider when answering these questions.
                When writing your review, please keep in mind that after decisions have been made, reviews and meta-reviews of accepted papers and opted-in rejected papers will be made public.

                1. Summary: Briefly summarize the paper and its contributions. This is not the place to critique the paper; the authors should generally agree with a well-written summary.
                  - Strengths and Weaknesses: Please provide a thorough assessment of the strengths and weaknesses of the paper, touching on each of the following dimensions:
                  - Originality: Are the tasks or methods new? Is the work a novel combination of well-known techniques? (This can be valuable!) Is it clear how this work differs from previous contributions? Is related work adequately cited
                  - Quality: Is the submission technically sound? Are claims well supported (e.g., by theoretical analysis or experimental results)? Are the methods used appropriate? Is this a complete piece of work or work in progress? Are the authors careful and honest about evaluating both the strengths and weaknesses of their work
                  - Clarity: Is the submission clearly written? Is it well organized? (If not, please make constructive suggestions for improving its clarity.) Does it adequately inform the reader? (Note that a superbly written paper provides enough information for an expert reader to reproduce its results.)
                  - Significance: Are the results important? Are others (researchers or practitioners) likely to use the ideas or build on them? Does the submission address a difficult task in a better way than previous work? Does it advance the state of the art in a demonstrable way? Does it provide unique data, unique conclusions about existing data, or a unique theoretical or experimental approach?

                2. Questions: Please list up and carefully describe any questions and suggestions for the authors. Think of the things where a response from the author can change your opinion, clarify a confusion or address a limitation. This can be very important for a productive rebuttal and discussion phase with the authors.

                3. Limitations: Have the authors adequately addressed the limitations and potential negative societal impact of their work? If not, please include constructive suggestions for improvement.
                In general, authors should be rewarded rather than punished for being up front about the limitations of their work and any potential negative societal impact. You are encouraged to think through whether any critical points are missing and provide these as feedback for the authors.

                4. Ethical concerns: If there are ethical issues with this paper, please flag the paper for an ethics review. For guidance on when this is appropriate, please review the NeurIPS ethics guidelines.

                5. Soundness: Please assign the paper a numerical rating on the following scale to indicate the soundness of the technical claims, experimental and research methodology and on whether the central claims of the paper are adequately supported with evidence.
                  4: excellent
                  3: good
                  2: fair
                  1: poor

                6. Presentation: Please assign the paper a numerical rating on the following scale to indicate the quality of the presentation. This should take into account the writing style and clarity, as well as contextualization relative to prior work.
                  4: excellent
                  3: good
                  2: fair
                  1: poor

                7. Contribution: Please assign the paper a numerical rating on the following scale to indicate the quality of the overall contribution this paper makes to the research area being studied. Are the questions being asked important? Does the paper bring a significant originality of ideas and/or execution? Are the results valuable to share with the broader NeurIPS community.
                  4: excellent
                  3: good
                  2: fair
                  1: poor

                8. Overall: Please provide an "overall score" for this submission. Choices:
                  10: Award quality: Technically flawless paper with groundbreaking impact on one or more areas of AI, with exceptionally strong evaluation, reproducibility, and resources, and no unaddressed ethical considerations.
                  9: Very Strong Accept: Technically flawless paper with groundbreaking impact on at least one area of AI and excellent impact on multiple areas of AI, with flawless evaluation, resources, and reproducibility, and no unaddressed ethical considerations.
                  8: Strong Accept: Technically strong paper, with novel ideas, excellent impact on at least one area of AI or high-to-excellent impact on multiple areas of AI, with excellent evaluation, resources, and reproducibility, and no unaddressed ethical considerations.
                  7: Accept: Technically solid paper, with high impact on at least one sub-area of AI or moderate-to-high impact on more than one area of AI, with good-to-excellent evaluation, resources, reproducibility, and no unaddressed ethical considerations.
                  6: Weak Accept: Technically solid, moderate-to-high impact paper, with no major concerns with respect to evaluation, resources, reproducibility, ethical considerations.
                  5: Borderline accept: Technically solid paper where reasons to accept outweigh reasons to reject, e.g., limited evaluation. Please use sparingly.
                  4: Borderline reject: Technically solid paper where reasons to reject, e.g., limited evaluation, outweigh reasons to accept, e.g., good evaluation. Please use sparingly.
                  3: Reject: For instance, a paper with technical flaws, weak evaluation, inadequate reproducibility and incompletely addressed ethical considerations.
                  2: Strong Reject: For instance, a paper with major technical flaws, and/or poor evaluation, limited impact, poor reproducibility and mostly unaddressed ethical considerations.
                  1: Very Strong Reject: For instance, a paper with trivial results or unaddressed ethical considerations

                9. Confidence:  Please provide a "confidence score" for your assessment of this submission to indicate how confident you are in your evaluation. Choices:
                  5: You are absolutely certain about your assessment. You are very familiar with the related work and checked the math/other details carefully.
                  4: You are confident in your assessment, but not absolutely certain. It is unlikely, but not impossible, that you did not understand some parts of the submission or that you are unfamiliar with some pieces of related work.
                  3: You are fairly confident in your assessment. It is possible that you did not understand some parts of the submission or that you are unfamiliar with some pieces of related work. Math/other details were not carefully checked.
                  2: You are willing to defend your assessment, but it is quite likely that you did not understand the central parts of the submission or that you are unfamiliar with some pieces of related work. Math/other details were not carefully checked.
                  1: Your assessment is an educated guess. The submission is not in your area or the submission was difficult to understand. Math/other details were not carefully checked.

                  You must make sure that all sections are properly created: abstract, introduction, methods, results, and discussion. Points must be reduced from your scores if any of these are missing.
                """
                + template_instructions
            )
            if reviewer_type is None:
                reviewer_type = ""
            sys_prompt_content = (
                "You are an AI researcher who is reviewing a paper that was submitted to a prestigious ML venue. "
                f"Be critical and cautious in your decision. {reviewer_type}\n"
            ) + neurips_form
            prompt_content = (
                f"Outlined in the following text is the research plan that the machine learning engineer was tasked with building: {outlined_plan}\n\n"
                f"The following text is the research latex that the model produced: \n{latex}\n\n"
            )

            scoring_output = query_model(
                system_prompt=sys_prompt_content,
                gemini_api_key=gemini_api_key,
                prompt=prompt_content,
            )
            if scoring_output is None:  # query_model might return None
                raise ValueError("LLM query returned None.")

            review_json = extract_json_between_markers(scoring_output)

            if review_json is None:
                raise ValueError("Could not extract JSON review from LLM output.")

            # Ensure all required keys are present
            required_keys = [
                "Overall",
                "Soundness",
                "Confidence",
                "Contribution",
                "Presentation",
                "Clarity",
                "Originality",
                "Quality",
                "Significance",
            ]
            for key in required_keys:
                if key not in review_json:
                    raise KeyError(f"Missing key '{key}' in review JSON.")

            overall = int(review_json["Overall"]) / 10.0
            soundness = int(review_json["Soundness"]) / 4.0
            confidence = int(review_json["Confidence"]) / 5.0
            contribution = int(review_json["Contribution"]) / 4.0
            presentation = int(review_json["Presentation"]) / 4.0
            clarity = int(review_json["Clarity"]) / 4.0
            originality = int(review_json["Originality"]) / 4.0
            quality = int(review_json["Quality"]) / 4.0
            significance = int(review_json["Significance"]) / 4.0

            clarity_weight = 0.1
            quality_weight = 0.1
            overall_weight = 1.0
            soundness_weight = 0.1
            confidence_weight = 0.1
            originality_weight = 0.1
            significance_weight = 0.1
            contribution_weight = 0.4
            presentation_weight = 0.2

            # max possible score based on weights
            max_possible_weighted_score = (
                clarity_weight
                + quality_weight
                + overall_weight
                + soundness_weight
                + confidence_weight
                + originality_weight
                + significance_weight
                + contribution_weight
                + presentation_weight
            )

            if max_possible_weighted_score == 0:  # Avoid division by zero
                performance = 0.0
            else:
                performance = (
                    (
                        soundness_weight * soundness
                        + presentation_weight * presentation
                        + confidence_weight * confidence
                        + contribution_weight * contribution
                        + overall_weight * overall
                        + originality_weight * originality
                        + significance * significance_weight
                        + clarity_weight * clarity
                        + quality_weight * quality
                    )
                    / max_possible_weighted_score
                ) * 10.0  # Scale to 0-10
            return (
                performance,
                f"The performance of your submission is: {performance}\n{scoring_output}",
                True,
            )
        except (ValueError, KeyError, TypeError, Exception) as e:  # Catch more specific errors too
            print(f"Error in get_score (attempt {_attempt + 1}/{attempts}): {e}")
            last_exception_message = str(e)
    return (
        None,
        f"Failed to get score after {attempts} attempts. Last error: {last_exception_message}",
        False,
    )


class ReviewersAgent:
    """An agent that simulates multiple reviewers providing feedback on a research plan and report.

    This agent invokes the `get_score` function multiple times, each with a
    different reviewer persona, to gather a diverse set of reviews for a given
    research output.

    Args:
        notes (Optional[List[Any]]): A list of notes, currently not used by this agent
                                     but kept for consistency with BaseAgent. Defaults to None.
        gemini_api_key (Optional[str]): API key for the Gemini model. Defaults to None.
    """

    def __init__(self, notes: list[Any] | None = None, gemini_api_key: str | None = None):
        """Initializes the ReviewersAgent.

        Args:
            notes: Optional list of notes (currently unused).
            gemini_api_key: API key for the Gemini model.
        """
        if notes is None:
            self.notes: list[Any] = []
        else:
            self.notes = notes
        self.gemini_api_key = gemini_api_key

    def inference(self, plan: str, report: str) -> str:
        """Generates reviews from multiple simulated reviewers.

        Args:
            plan: The research plan.
            report: The research report (in LaTeX format).

        Returns:
            A string concatenating the feedback from all reviewers.
        """
        reviewer_1_persona = "You are a harsh but fair reviewer and expect good experiments that lead to insights for the research topic."
        review_1 = get_score(
            outlined_plan=plan,
            latex=report,
            reviewer_type=reviewer_1_persona,
            gemini_api_key=self.gemini_api_key,
        )

        reviewer_2_persona = "You are a harsh and critical but fair reviewer who is looking for an idea that would be impactful in the field."
        review_2 = get_score(
            outlined_plan=plan,
            latex=report,
            reviewer_type=reviewer_2_persona,
            gemini_api_key=self.gemini_api_key,
        )

        reviewer_3_persona = "You are a harsh but fair open-minded reviewer that is looking for novel ideas that have not been proposed before."
        review_3 = get_score(
            outlined_plan=plan,
            latex=report,
            reviewer_type=reviewer_3_persona,
            gemini_api_key=self.gemini_api_key,
        )

        return f"Reviewer #1:\n{review_1}, \nReviewer #2:\n{review_2}, \nReviewer #3:\n{review_3}"


class BaseAgent:
    """Abstract base class for agents in the research simulation.

    Provides common functionalities like:
    - Storing notes and API keys.
    - Managing interaction history.
    - A basic inference loop using `query_model`.
    - Cleaning text output.

    Subclasses must implement methods to define their specific role,
    prompts for different phases, context generation, available commands,
    and example commands.

    Attributes:
        notes (List[Any]): A list of notes for the agent.
        max_steps (int): Maximum number of steps for an interaction.
        gemini_api_key (Optional[str]): API key for the Gemini model.
        phases (List[str]): List of phases this agent can operate in.
        plan (str): Current research plan.
        report (str): Current research report.
        history (List[Tuple[Optional[int], str]]): History of interactions.
        prev_comm (str): The previous command issued by the agent.
        prev_report (str): The previous version of the report.
        lit_review_sum (str): Summary of the literature review.
        reviewer_response (str): Response from reviewers.
        prev_interpretation (str): Previous interpretation of results.
        second_round (bool): Flag indicating if it's a second round of interaction.
        max_hist_len (int): Maximum length of the history to keep.
    """

    def __init__(
        self,
        notes: list[Any] | None = None,
        max_steps: int = 100,
        gemini_api_key: str | None = None,
    ):
        """Initializes the BaseAgent.

        Args:
            notes: Optional list of notes.
            max_steps: Maximum number of interaction steps.
            gemini_api_key: API key for the Gemini model.
        """
        if notes is None:
            self.notes: list[Any] = []
        else:
            self.notes = notes
        self.max_steps = max_steps
        self.phases: list[str] = []
        self.plan: str = ""
        self.report: str = ""
        self.history: list[tuple[int | None, str]] = []
        self.prev_comm: str = ""
        self.prev_report: str = ""
        self.lit_review_sum: str = ""
        self.reviewer_response: str = ""
        self.prev_interpretation: str = ""
        self.gemini_api_key = gemini_api_key

        self.second_round: bool = False
        self.max_hist_len: int = 15

    @staticmethod
    def clean_text(text: str) -> str:
        """Performs minor corrections on text, like fixing markdown code block inconsistencies.

        Args:
            text: The input string.

        Returns:
            The corrected string.
        """
        text = text.replace("```\n", "```")
        return text

    def override_inference(self, query: str) -> str | None:
        """Performs an LLM inference with a system prompt based on the agent's role.

        Args:
            query: The user prompt for the LLM.
            temp: Temperature setting for the LLM (currently unused in `query_model` call).

        Returns:
            The LLM's response string, or None if query_model fails.
        """
        sys_prompt = f"""You are {self.role_description()}"""
        # `temp` is not used in the provided `query_model` signature
        model_resp = query_model(
            system_prompt=sys_prompt, prompt=query, gemini_api_key=self.gemini_api_key
        )
        return model_resp

    def inference(
        self,
        research_topic: str,
        phase: str,
        step: int,
        feedback: str = "",
    ) -> str | None:
        """Main inference method for the agent.

        Constructs a detailed prompt including context, history, task instructions,
        and current step information, then queries the LLM.
        Manages the interaction history, including expiration of historical items.

        Args:
            research_topic: The main research topic.
            phase: The current research phase (e.g., "plan formulation").
            step: The current step number in the interaction.
            feedback: Feedback from a previous step or another agent.

        Returns:
            The LLM's response string after cleaning, or None if query_model fails.
        """
        sys_prompt = f"""You are {self.role_description()} \nTask instructions: {self.phase_prompt(phase)}\n{self.command_descriptions(phase)}"""
        current_context = self.context(phase)
        history_str = "\n".join([item[1] for item in self.history])  # item is (Optional[int], str)
        phase_notes = [
            _note
            for _note in self.notes
            if isinstance(_note, dict) and phase in _note.get("phases", [])
        ]
        notes_str = f"Notes for the task objective: {phase_notes}\n" if len(phase_notes) > 0 else ""
        complete_str = ""
        if (self.max_steps - 1) > 0 and step / (
            self.max_steps - 1
        ) > 0.7:  # Avoid division by zero if max_steps is 1
            complete_str = "You must finish this task and submit as soon as possible!"

        prompt = (
            f"""{current_context}\n{"~" * 10}\nHistory: {history_str}\n{"~" * 10}\n"""
            f"Current Step #{step}, Phase: {phase}\n{complete_str}\n"
            f"[Objective] Your goal is to perform research on the following topic: {research_topic}\n"
            f"Feedback: {feedback}\nNotes: {notes_str}\nYour previous command was: {self.prev_comm}. Make sure your new output is very different.\nPlease produce a single command below:\n"
        )
        # `temp` is not used in the provided `query_model` signature
        model_resp = query_model(
            system_prompt=sys_prompt, prompt=prompt, gemini_api_key=self.gemini_api_key
        )
        if model_resp is None:
            return None

        print("^" * 50, phase, "^" * 50)
        model_resp = self.clean_text(model_resp)
        self.prev_comm = model_resp
        steps_exp = None
        if feedback is not None and "```EXPIRATION" in feedback:
            try:
                steps_exp_str = feedback.split("\n")[0].replace("```EXPIRATION ", "")
                steps_exp = int(steps_exp_str)
                # Assuming extract_prompt is available and works as intended
                feedback_extracted = extract_prompt(feedback, "EXPIRATION")
                if feedback_extracted is not None:  # use extracted feedback if available
                    feedback = feedback_extracted
            except (ValueError, IndexError):
                # If parsing expiration fails, keep original feedback and no expiration.
                steps_exp = None

        self.history.append(
            (
                steps_exp,
                f"Step #{step}, Phase: {phase}, Feedback: {feedback}, Your response: {model_resp}",
            )
        )
        # remove histories that have expiration dates
        new_history = []
        for i in range(len(self.history)):
            expiration, entry_str = self.history[i]
            if expiration is not None:
                expiration -= 1
                if expiration >= 0:
                    new_history.append((expiration, entry_str))
            else:
                new_history.append((expiration, entry_str))
        self.history = new_history

        if len(self.history) >= self.max_hist_len:
            self.history.pop(0)  # Remove the oldest entry
        return model_resp

    def reset(self) -> None:
        """Resets the agent's history and previous command."""
        self.history.clear()
        self.prev_comm = ""

    def context(self, phase: str) -> str | tuple[str, ...]:
        """Generates context relevant to the current phase for the LLM prompt.

        This method should be implemented by subclasses.

        Args:
            phase: The current research phase.

        Returns:
            A string or tuple of strings representing the context.
        """
        raise NotImplementedError(
            "Subclasses should implement this method to provide phase-specific context."
        )

    def phase_prompt(self, phase: str) -> str:
        """Returns the specific instructions or goals for the agent in the given phase.

        This method should be implemented by subclasses.

        Args:
            phase: The current research phase.

        Returns:
            A string containing the phase-specific prompt.
        """
        raise NotImplementedError(
            "Subclasses should implement this method to provide phase-specific prompts."
        )

    def role_description(self) -> str:
        """Returns a string describing the agent's role.

        This method should be implemented by subclasses.

        Returns:
            A string describing the agent's role.
        """
        raise NotImplementedError(
            "Subclasses should implement this method to provide a role description."
        )

    def command_descriptions(self, phase: str) -> str:
        """Returns a string describing the available commands for the agent in a given phase.

        This method should be implemented by subclasses.

        Args:
            phase: The current research phase.

        Returns:
            A string describing available commands.
        """
        raise NotImplementedError(
            "Subclasses should implement this method to describe available commands."
        )

    def example_command(self, phase: str) -> str | tuple[()]:  # Tuple[()] for empty tuple
        """Returns an example of a command for the agent in a given phase.

        This method should be implemented by subclasses.

        Args:
            phase: The current research phase.

        Returns:
            A string or an empty tuple representing an example command.
        """
        raise NotImplementedError(
            "Subclasses should implement this method to provide example commands."
        )


class ProfessorAgent(BaseAgent):
    """An agent representing a Computer Science Professor.

    This agent is primarily involved in the "report writing" phase, guiding a PhD
    student to write a LaTeX report based on experimental results. It can also
    generate a README.md for a GitHub repository.

    Inherits from `BaseAgent`.
    """

    def __init__(
        self,
        notes: list[Any] | None = None,
        max_steps: int = 100,
        gemini_api_key: str | None = None,
    ):
        """Initializes the ProfessorAgent.

        Args:
            notes: Optional list of notes.
            max_steps: Maximum number of interaction steps.
            gemini_api_key: API key for the Gemini model.
        """
        super().__init__(notes, max_steps, gemini_api_key)
        self.phases = ["report writing"]

    def generate_readme(self) -> str | None:
        """Generates a README.md file content based on the project's history and report.

        Returns:
            A string containing the generated README.md content in markdown format,
            or None if the LLM query fails.
        """
        sys_prompt = f"""You are {self.role_description()} \n Here is the written paper \n{self.report}. Task instructions: Your goal is to integrate all of the knowledge, code, reports, and notes provided to you and generate a readme.md for a github repository."""
        history_str = "\n".join([item[1] for item in self.history])
        prompt = (
            f"""History: {history_str}\n{"~" * 10}\n"""
            f"Please produce the readme below in markdown:\n"
        )
        model_resp = query_model(
            system_prompt=sys_prompt, prompt=prompt, gemini_api_key=self.gemini_api_key
        )
        if model_resp is None:
            return None
        return (
            model_resp.replace("```markdown", "").replace("```", "").strip()
        )  # Clean up markdown tags

    def context(self, phase: str) -> str | tuple[str, str]:
        """Provides context for the "report writing" phase.

        Includes information from previous rounds if applicable (previous interpretation,
        report, reviewer responses) and current artifacts (literature review, plan).

        Args:
            phase: The current research phase (should be "report writing").

        Returns:
            A string or tuple of strings with contextual information.
            Returns an empty string if the phase is not "report writing".
        """
        sr_str_parts: list[str] = []
        if self.second_round:
            sr_str_parts.append("The following are results from the previous experiments\n")
            sr_str_parts.append(f"Previous Interpretation of results: {self.prev_interpretation}\n")
            sr_str_parts.append(f"Previous Report: {self.prev_report}\n")
            sr_str_parts.append(f"{self.reviewer_response}\n\n\n")

        sr_str = "".join(sr_str_parts)

        if phase == "report writing":
            # Ensure tuple elements are strings
            return (
                sr_str,
                f"Current Literature Review: {self.lit_review_sum}\nCurrent Plan: {self.plan}\n",
            )
        return ""  # Default for other phases, though this agent is mainly for "report writing"

    def example_command(self, phase: str) -> str:
        """Provides example commands for the "report writing" phase.

        Args:
            phase: The current research phase.

        Returns:
            A string with example commands.

        Raises:
            Exception: If the phase is not "report writing".
        """
        if phase not in self.phases:
            raise Exception(f"Invalid phase for ProfessorAgent: {phase}. Expected: {self.phases}")
        return (
            "You can produce dialogue using the following command: ```DIALOGUE\ndialogue here\n```\n where dialogue here is the actual dialogue you will send and DIALOGUE is just the word DIALOGUE.\n"
            "When performing a command, make sure to include the three ticks (```) at the top and bottom ```COMMAND\n<Insert command here>\n``` where COMMAND is the specific command you want to run (e.g. REPORT, DIALOGUE).\n"
        )

    def command_descriptions(self, phase: str) -> str:
        """Describes available commands for the "report writing" phase.

        Focuses on the `LATEX` command for submitting the final report.

        Args:
            phase: The current research phase.

        Returns:
            A string describing commands.

        Raises:
            Exception: If the phase is not "report writing".
        """
        if phase not in self.phases:
            raise Exception(f"Invalid phase for ProfessorAgent: {phase}. Expected: {self.phases}")
        return (
            "When you believe a good report has been arrived at between you and the PhD student you can use the following command to end the dialogue and submit the plan ```LATEX\nreport here\n```\n where report here is the actual report written in compilable latex to be transmitted and LATEX is just the word LATEX.\n"
            "Your report should include numbers, relevant metrics to the experiment (e.g. accuracy or loss) and measures of significance. You must propagate this information accurately. You must also submit the report promptly. Do not delay too long.\n"
            "You must be incredibly detailed about what you did for the experiment and all of the findings.\n"
        )

    def phase_prompt(self, phase: str) -> str:
        """Provides the main prompt/goal for the "report writing" phase.

        Args:
            phase: The current research phase.

        Returns:
            A string defining the agent's goal for the phase.

        Raises:
            Exception: If the phase is not "report writing".
        """
        if phase not in self.phases:
            raise Exception(f"Invalid phase for ProfessorAgent: {phase}. Expected: {self.phases}")
        phase_str = (
            "You are directing a PhD student to help them write a report in latex based on results from an experiment, and you interact with them through dialogue.\n"
            "Your goal is to write a report in latex for an experiment. You should read through the code, read through the interpretation, and look at the results to understand what occurred. You should then discuss with the PhD student how they can write up the results and give their feedback to improve their thoughts.\n"
        )
        return phase_str

    def role_description(self) -> str:
        """Describes the agent's role.

        Returns:
            A string: "a computer science professor at a top university."
        """
        return "a computer science professor at a top university."


class PostdocAgent(BaseAgent):
    """An agent representing a Computer Science Postdoctoral Researcher.

    This agent is involved in "plan formulation" and "results interpretation" phases.
    It guides a PhD student in developing research plans and interpreting
    experimental results.

    Inherits from `BaseAgent`.
    """

    def __init__(
        self,
        notes: list[Any] | None = None,
        max_steps: int = 100,
        gemini_api_key: str | None = None,
    ):
        """Initializes the PostdocAgent.

        Args:
            notes: Optional list of notes.
            max_steps: Maximum number of interaction steps.
            gemini_api_key: API key for the Gemini model.
        """
        super().__init__(notes, max_steps, gemini_api_key)
        self.phases = ["plan formulation", "results interpretation"]

    def context(self, phase: str) -> str | tuple[str, str]:
        """Provides context for "plan formulation" or "results interpretation" phases.

        Includes information from previous rounds if applicable and current artifacts
        (literature review, plan).

        Args:
            phase: The current research phase.

        Returns:
            A string or tuple of strings with contextual information.
            Returns an empty string if the phase is not recognized.
        """
        sr_str_parts: list[str] = []
        if self.second_round:
            sr_str_parts.append("The following are results from the previous experiments\n")
            sr_str_parts.append(f"Previous Interpretation of results: {self.prev_interpretation}\n")
            sr_str_parts.append(f"Previous Report: {self.prev_report}\n")
            sr_str_parts.append(f"{self.reviewer_response}\n\n\n")
        sr_str = "".join(sr_str_parts)

        if phase == "plan formulation":
            return (
                sr_str,
                f"Current Literature Review: {self.lit_review_sum}",
            )
        elif phase == "results interpretation":
            return (
                sr_str,
                f"Current Literature Review: {self.lit_review_sum}\nCurrent Plan: {self.plan}\n",
            )
        return ""

    def example_command(self, phase: str) -> tuple[()]:
        """Provides example commands for the agent's phases.

        Currently returns an empty tuple, indicating no specific complex example is provided.

        Args:
            phase: The current research phase.

        Returns:
            An empty tuple.

        Raises:
            Exception: If the phase is not "plan formulation" or "results interpretation".
        """
        if phase not in self.phases:
            raise Exception(
                f"Invalid phase for PostdocAgent: {phase}. Expected one of: {self.phases}"
            )
        return ()  # No specific example command structure beyond what's in command_descriptions

    def command_descriptions(self, phase: str) -> str:
        """Describes available commands for "plan formulation" or "results interpretation".

        - For "plan formulation": `DIALOGUE` and `PLAN`.
        - For "results interpretation": `DIALOGUE` and `INTERPRETATION`.

        Args:
            phase: The current research phase.

        Returns:
            A string describing commands for the given phase.

        Raises:
            Exception: If the phase is not recognized.
        """
        if phase not in self.phases:
            raise Exception(
                f"Invalid phase for PostdocAgent: {phase}. Expected one of: {self.phases}"
            )
        if phase == "plan formulation":
            return (
                "You can produce dialogue using the following command: ```DIALOGUE\ndialogue here\n```\n where dialogue here is the actual dialogue you will send and DIALOGUE is just the word DIALOGUE.\n"
                "When you believe a good plan has been arrived at between you and the PhD student you can use the following command to end the dialogue and submit the plan ```PLAN\nplan here\n```\n where plan here is the actual plan to be transmitted and PLAN is just the word PLAN. Plan here should provide a clear outline for how to achieve the task, including what machine learning models to use and implement, what types of datasets should be searched for and used to train the model, and the exact details of the experiment.\n"
                "You can only use a SINGLE command per inference turn. Do not use more than one command per inference. If you use multiple commands, then only one of them will be executed, NOT BOTH.\n"
                "Make sure not to produce too much dialogue and to submit an plan in reasonable time."
                "When performing a command, make sure to include the three ticks (```) at the top and bottom ```COMMAND\ntext\n``` where COMMAND is the specific command you want to run (e.g. PLAN, DIALOGUE).\n"
            )
        elif phase == "results interpretation":
            return (
                "When you believe a good interpretation has been arrived at between you and the PhD student you can use the following command to end the dialogue and submit the plan ```INTERPRETATION\ninterpretation here\n```\n where interpretation here is the actual interpretation to be transmitted and INTERPRETATION is just the word INTERPRETATION. Please provide an INTERPRETATION in a reasonable amount of time.\n"
                "You can produce dialogue using the following command: ```DIALOGUE\ndialogue here\n```\n where dialogue here is the actual dialogue you will send and DIALOGUE is just the word DIALOGUE.\n"
                "You must submit the interpretation during this phase in a reasonable amount of time. Do not delay the submission."
                "When performing a command, make sure to include the three ticks (```) at the top and bottom ```COMMAND\ntext\n``` where COMMAND is the specific command you want to run (e.g. INTERPRETATION, DIALOGUE).\n"
            )
        return ""  # Should not be reached if phase validation is done

    def phase_prompt(self, phase: str) -> str:
        """Provides the main prompt/goal for "plan formulation" or "results interpretation".

        Args:
            phase: The current research phase.

        Returns:
            A string defining the agent's goal for the phase.

        Raises:
            Exception: If the phase is not recognized.
        """
        if phase not in self.phases:
            raise Exception(
                f"Invalid phase for PostdocAgent: {phase}. Expected one of: {self.phases}"
            )
        phase_str = ""
        if phase == "plan formulation":
            phase_str = (
                "You are directing a PhD student to help them come up with a good plan, and you interact with them through dialogue.\n"
                "Your goal is to produce plans that would make good experiments for the given topic. You should aim for a very simple experiment that showcases your plan, not a complex one. You should integrate the provided literature review and come up with plans on how to expand and build on these works for the given topic. Your plans should provide a clear outline for how to achieve the task, including what machine learning models to use and implement, what types of datasets should be searched for and used to train the model, and the exact details of the experiment. Your idea should be very innovative and unlike anything seen before.\n"
            )
        elif phase == "results interpretation":
            phase_str = (
                "You are directing a PhD student to help them come up with an interpretation for results from an experiment, and you interact with them through dialogue.\n"
                "Your goal is to interpret results from experiments that were previously run. You should read through the code and look at the results to understand what occurred. You should then discuss with the PhD student how they can interpret the results and give their feedback to improve their thoughts. You should integrate the provided literature review, code, and plans to come up with an exciting interpretation that could make a compelling paper. Your plans should provide a clear outline that can be used to write an academic paper.\n"
                "Your interpretation should include numbers, relevant metrics to the experiment (e.g. accuracy or loss) and measures of significance. You must propagate this information accurately. You must also complete this in a reasonable amount of time and then submit your results.\n"
            )
        return phase_str

    def role_description(self) -> str:
        """Describes the agent's role.

        Returns:
            A string: "a computer science postdoctoral student at a top university."
        """
        return "a computer science postdoctoral student at a top university."


class PhDStudentAgent(BaseAgent):
    """An agent representing a Computer Science PhD Student.

    This agent is involved in multiple research phases:
    - "literature review": Searching and summarizing academic papers.
    - "plan formulation": Collaborating on research plans.
    - "report writing": (Partially defined, phase prompt commented out in original)
    - "report refinement": Responding to reviewer feedback.
    - "data preparation": (Partially defined, phase prompt commented out in original)

    It has specific commands for interacting with an arXiv-like service and
    can generate a `requirements.txt` file.

    Inherits from `BaseAgent`.

    Attributes:
        lit_review (List[Dict[str, str]]): A list of dictionaries, where each
                                          dictionary stores details of a paper
                                          in the literature review (arxiv_id,
                                          full_text, summary).
    """

    def __init__(
        self,
        notes: list[Any] | None = None,
        max_steps: int = 100,
        gemini_api_key: str | None = None,
    ):
        """Initializes the PhDStudentAgent.

        Args:
            notes: Optional list of notes.
            max_steps: Maximum number of interaction steps.
            gemini_api_key: API key for the Gemini model.
        """
        super().__init__(notes, max_steps, gemini_api_key)
        self.phases = [
            "literature review",
            "plan formulation",
            "report writing",  # Note: phase_prompt for this is commented out in original
            "report refinement",
            # "data preparation" was also in command_descriptions/phase_prompt originally
        ]
        self.lit_review: list[dict[str, str]] = []

    def context(self, phase: str) -> str | tuple[str, str]:
        """Provides context for the PhD student's current phase.

        Context can include previous experimental results, interpretations, reports,
        reviewer responses, literature review summaries, and current plans,
        depending on the phase and whether it's a "second round" of interaction.

        Args:
            phase: The current research phase.

        Returns:
            A string or tuple of strings with contextual information.
            Returns an empty string if the phase is not specifically handled.
        """
        sr_str_parts: list[str] = []
        if self.second_round:
            sr_str_parts.append("The following are results from the previous experiments\n")
            sr_str_parts.append(f"Previous Interpretation of results: {self.prev_interpretation}\n")
            sr_str_parts.append(f"Previous Report: {self.prev_report}\n")
            sr_str_parts.append(f"{self.reviewer_response}\n\n\n")
        sr_str = "".join(sr_str_parts)

        if phase == "plan formulation":
            return (
                sr_str,
                f"Current Literature Review: {self.lit_review_sum}",
            )
        elif phase == "data preparation":  # This phase was in original command descriptions
            return (
                sr_str,
                f"Current Literature Review: {self.lit_review_sum}\nCurrent Plan: {self.plan}",
            )
        elif (
            phase == "results interpretation"
        ):  # This phase context was defined, but phase itself not in self.phases
            return (
                sr_str,
                f"Current Literature Review: {self.lit_review_sum}\nCurrent Plan: {self.plan}\n",
            )
        elif phase == "report refinement":
            return (
                sr_str,
                f"Current Literature Review: {self.lit_review_sum}\nCurrent Plan: {self.plan}\n",
            )
        elif phase == "literature review":
            return sr_str  # Just the second_round info if applicable
        else:
            return ""  # Default for other phases like "report writing" which has no specific context here

    def requirements_txt(self) -> str | None:
        """Generates a `requirements.txt` file content based on project history and code.

        Returns:
            A string containing the `requirements.txt` content,
            or None if the LLM query fails.
        """
        sys_prompt = f"""You are {self.role_description()} \nTask instructions: Your goal is to integrate all of the knowledge, code, reports, and notes provided to you and generate a requirements.txt for a github repository for all of the code."""
        history_str = "\n".join([item[1] for item in self.history])
        prompt = (
            f"""History: {history_str}\n{"~" * 10}\n"""
            f"Please produce the requirements.txt below in markdown:\n"
        )
        model_resp = query_model(
            system_prompt=sys_prompt, prompt=prompt, gemini_api_key=self.gemini_api_key
        )
        if model_resp is None:
            return None
        # Assuming the model might wrap it in markdown
        return model_resp.replace("```markdown", "").replace("```", "").strip()

    def example_command(self, phase: str) -> tuple[()]:
        """Provides example commands for the agent's phases.

        Currently returns an empty tuple, implying examples are within command descriptions.

        Args:
            phase: The current research phase.

        Returns:
            An empty tuple.

        Raises:
            Exception: If the phase is not recognized by the agent.
        """
        if phase not in self.phases:  # Add other phases if they get specific examples
            raise Exception(
                f"Invalid phase for PhDStudentAgent: {phase}. Expected one of: {self.phases}"
            )
        return ()

    def command_descriptions(self, phase: str) -> str:
        """Describes available commands for the PhD student in various phases.

        - "literature review": `SUMMARY`, `FULL_TEXT`, `ADD_PAPER`.
        - "plan formulation": `DIALOGUE`.
        - "data preparation": `DIALOGUE`, `SUBMIT_CODE`.
        - "results interpretation": `DIALOGUE`.
        - "report refinement": No specific commands listed (returns empty string).

        Args:
            phase: The current research phase.

        Returns:
            A string describing commands for the given phase. Empty if no commands.

        Raises:
            Exception: If the phase is not recognized by the agent.
        """
        # Validate phase first
        # Note: "report writing" command description was commented out in the original.
        # "results interpretation" was also present in original.
        valid_phases_for_commands = [
            "literature review",
            "plan formulation",
            "data preparation",
            "results interpretation",
            "report refinement",
        ] + self.phases  # ensure self.phases is source of truth
        if phase not in valid_phases_for_commands and phase not in self.phases:
            raise Exception(f"Invalid phase for PhDStudentAgent command descriptions: {phase}")

        if phase == "literature review":
            return (
                "To collect paper summaries, use the following command: ```SUMMARY\nSEARCH QUERY\n```\n where SEARCH QUERY is a string that will be used to find papers with semantically similar content and SUMMARY is just the word SUMMARY. Make sure your search queries are very short.\n"
                "To get the full paper text for an arXiv paper, use the following command: ```FULL_TEXT\narXiv paper ID\n```\n where arXiv paper ID is the ID of the arXiv paper (which can be found by using the SUMMARY command), and FULL_TEXT is just the word FULL_TEXT. Make sure to read the full text using the FULL_TEXT command before adding it to your list of relevant papers.\n"
                "If you believe a paper is relevant to the research project proposal, you can add it to the official review after reading using the following command: ```ADD_PAPER\narXiv_paper_ID\nPAPER_SUMMARY\n```\nwhere arXiv_paper_ID is the ID of the arXiv paper, PAPER_SUMMARY is a brief summary of the paper, and ADD_PAPER is just the word ADD_PAPER. You can only add one paper at a time. \n"
                "Make sure to use ADD_PAPER when you see a relevant paper. DO NOT use SUMMARY too many times."
                "You can only use a SINGLE command per inference turn. Do not use more than one command per inference. If you use multiple commands, then only one of them will be executed, not both.\n"
                "Make sure to extensively discuss the experimental results in your summary.\n"
                "When performing a command, make sure to include the three ticks (```) at the top and bottom ```COMMAND\ntext\n``` where COMMAND is the specific command you want to run (e.g. ADD_PAPER, FULL_TEXT, SUMMARY). Do not use the word COMMAND make sure to use the actual command, e.g. your command should look exactly like this: ```ADD_PAPER\ntext\n``` (where the command could be from ADD_PAPER, FULL_TEXT, SUMMARY)\n"
            )
        elif phase == "plan formulation":
            return (
                "You can produce dialogue using the following command: ```DIALOGUE\ndialogue here\n```\n where 'dialogue here' is the actual dialogue you will send and DIALOGUE is just the word DIALOGUE.\n"
                "You can only use a single command per inference turn. Do not use more than one command per inference. If you use multiple commands, then only one of them will be executed, not both.\n"
                "When performing a command, make sure to include the three ticks (```) at the top and bottom ```COMMAND\ntext\n``` where COMMAND is the specific command you want to run (e.g. DIALOGUE).\n"
            )
        elif phase == "data preparation":  # This phase was in original command descriptions
            return (
                "You can produce dialogue using the following command: ```DIALOGUE\ndialogue here\n```\n where 'dialogue here' is the actual dialogue you will send and DIALOGUE is just the word DIALOGUE.\n"
                "When you and the ML engineer have finalized your dataset preparation code and are ready to submit the final code, please use the following command: ```SUBMIT_CODE\ncode here\n```\n where 'code here' is the finalized code you will send and SUBMIT_CODE is just the word SUBMIT_CODE. Do not use any classes or functions. The submitted code must have a HuggingFace dataset import and must use an external HuggingFace dataset. If your code returns any errors, they will be provided to you, and you are also able to see print statements.  Make sure function variables are created inside the function or passed as a function parameter. DO NOT CREATE A MAIN FUNCTION.\n"
                "Make sure to submit code in a reasonable amount of time. Do not make the code too complex, try to make it simple. Do not take too long to submit code. Submit the code early. You should submit the code ASAP.\n"
                "You can only use a single command per inference turn. Do not use more than one command per inference. If you use multiple commands, then only one of them will be executed, not both.\n"
                "When performing a command, make sure to include the three ticks (```) at the top and bottom ```COMMAND\ntext\n``` where COMMAND is the specific command you want to run (e.g. SUBMIT_CODE, DIALOGUE).\n"
            )
        elif phase == "results interpretation":  # This phase was in original command descriptions
            return (
                "You can produce dialogue using the following command: ```DIALOGUE\ndialogue here\n```\n where 'dialogue here' is the actual dialogue you will send and DIALOGUE is just the word DIALOGUE.\n"
                "When performing a command, make sure to include the three ticks (```) at the top and bottom ```COMMAND\ntext\n``` where COMMAND is the specific command you want to run (e.g. DIALOGUE).\n"
            )
        # elif phase == "report writing": # Original was commented out
        #    return (
        #        "You can produce dialogue using the following command: ```DIALOGUE\ndialogue here\n```\n where 'dialogue here' is the actual dialogue you will send and DIALOGUE is just the word DIALOGUE.\n"
        #        "When performing a command, make sure to include the three ticks (```) at the top and bottom ```COMMAND\ntext\n``` where COMMAND is the specific command you want to run (e.g. DIALOGUE).\n")
        elif phase == "report refinement":
            return ""  # No specific commands listed for this phase in original
        return ""  # Default for phases without specific command descriptions (e.g. "report writing" as per original)

    def phase_prompt(self, phase: str) -> str:
        """Provides the main prompt/goal for the PhD student in various phases.

        Args:
            phase: The current research phase.

        Returns:
            A string defining the agent's goal for the phase.

        Raises:
            Exception: If the phase is not recognized by the agent.
        """
        # Validate phase first (include "data preparation", "results interpretation" as they were in original)
        valid_phases_for_prompt = self.phases + ["data preparation", "results interpretation"]
        if phase not in valid_phases_for_prompt:
            raise Exception(f"Invalid phase for PhDStudentAgent phase prompt: {phase}")

        phase_str = ""
        if phase == "literature review":
            phase_str = (
                "Your goal is to perform a literature review for the presented task and add papers to the literature review.\n"
                "You have access to arXiv and can perform two search operations: (1) finding many different paper summaries from a search query and (2) getting a single full paper text for an arXiv paper.\n"
            )
            reviewed_papers_ids = [_paper.get("arxiv_id", "N/A") for _paper in self.lit_review]
            rev_papers = "Papers in your review so far: " + " ".join(reviewed_papers_ids)
            phase_str += rev_papers if len(self.lit_review) > 0 else ""
        elif phase == "plan formulation":
            phase_str = (
                "You are a PhD student being directed by a postdoc who will help you come up with a good plan, and you interact with them through dialogue.\n"
                "Your goal is to produce plans that would make good experiments for the given topic. You should aim for a very simple experiment that showcases your plan, not a complex one. You should integrate the provided literature review and come up with plans on how to expand and build on these works for the given topic. Your plans should provide a clear outline for how to achieve the task, including what machine learning models to use and implement, what types of datasets should be searched for and used to train the model, and the exact details of the experiment. Your idea should be very innovative and unlike anything seen before.\n"
            )
        elif phase == "results interpretation":  # This prompt was in original
            phase_str = (
                "You are a PhD student being directed by a postdoc who will help you come up with an interpretation for results from an experiment, and you interact with them through dialogue.\n"
                "Your goal is to interpret results from experiments that were previously run. You should read through the code and look at the results to understand what occurred. You should then discuss with the postdoc your interpretation and use their feedback to improve your thoughts. You should integrate the provided literature review, code, and plans to come up with an exciting interpretation that could make a compelling paper. Your plans should provide a clear outline that can be used to write an academic paper.\n"
                "Your interpretation should include numbers, relevant metrics to the experiment (e.g. accuracy or loss) and measures of significance. You must propagate this information accurately.\n"
                "You must submit the interpretation during this phase in a reasonable amount of time. Do not delay the submission."
            )
        # elif phase == "report writing": # Original was commented out
        #    phase_str = (
        #        "You are a PhD student being directed by a professor who will help you write a report based on results from an experiment, and you interact with them through dialogue.\n"
        #        "Your goal is to write a report for an experiment entirely in latex. You should read through the code, read through the interpretation, and look at the results to understand what occurred. You should then discuss with the professor how you can write up the results and receive their feedback to improve your thoughts.\n"
        #        "Your report should include numbers, relevant metrics to the experiment (e.g. accuracy or loss) and measures of significance  in latex. You must propagate this information accurately.\n"
        #        "You must be incredibly detailed about what you did for the experiment and all of the findings.\n"
        #    )
        elif phase == "report refinement":
            phase_str = "You are a PhD student who has submitted their paper to an ML conference called ICLR. Your goal was to write a research paper and get high scores from the reviewers so that it get accepted to the conference.\n"
        # elif phase == "data preparation": # Original did not have a phase_prompt for this
        # phase_str = "..."
        return phase_str

    def role_description(self) -> str:
        """Describes the agent's role.

        Returns:
            A string: "a computer science PhD student at a top university."
        """
        return "a computer science PhD student at a top university."

    def add_review(
        self,
        review: str,
        arx_eng: Any,  # ArxivEngine-like object, type Any for flexibility
        agentrxiv: bool = False,
        global_agentrxiv: Any | None = None,  # AgentArxiv-like object
    ) -> tuple[str, str]:
        """Adds a paper to the agent's literature review list.

        Parses the review input (expected to be arXiv ID and summary),
        retrieves the full text (either via `arx_eng` or `global_agentrxiv`),
        and stores the information.

        Args:
            review: A string containing the paper's arXiv ID and summary,
                    typically separated by a newline.
            arx_eng: An object/engine to retrieve full paper text from arXiv.
                     It should have a `retrieve_full_paper_text(arxiv_id)` method.
            agentrxiv: Boolean flag, if True, uses `global_agentrxiv` for text retrieval.
            global_agentrxiv: An alternative engine for retrieving full text,
                              used if `agentrxiv` is True. It should have a
                              `retrieve_full_text(arxiv_id)` method.

        Returns:
            A tuple:
            - str: A message indicating success or error.
            - str: The full text of the paper, or an empty string on error.
        """
        try:
            arxiv_id: str
            review_text: str
            full_text: str | None

            if agentrxiv:
                if global_agentrxiv is None:
                    return "Error: agentrxiv is True but GLOBAL_agentrxiv is not provided.", ""
                parts = review.strip().split("\n", 1)
                if not parts:
                    raise ValueError("Review input is empty for agentrxiv.")
                arxiv_id = parts[0]
                review_text = parts[1] if len(parts) > 1 else ""
                full_text = global_agentrxiv.retrieve_full_text(arxiv_id)
            else:
                if arx_eng is None:
                    return "Error: arx_eng is not provided for standard arXiv retrieval.", ""
                parts = review.strip().split("\n", 1)
                if not parts:
                    raise ValueError("Review input is empty.")
                arxiv_id = parts[0]
                review_text = parts[1] if len(parts) > 1 else ""
                full_text = arx_eng.retrieve_full_paper_text(arxiv_id)

            if full_text is None:
                full_text = ""  # Ensure full_text is a string

            review_entry: dict[str, str] = {
                "arxiv_id": arxiv_id.strip(),
                "full_text": full_text,
                "summary": review_text.strip(),
            }
            self.lit_review.append(review_entry)
            return f"Successfully added paper {arxiv_id.strip()}", full_text
        except Exception as e:
            return (
                f"Error trying to add review -- bad formatting or retrieval issue: {str(e)}. "
                "Ensure the input format (ID newline Summary) is correct and the Arxiv ID is valid. "
                "A real paper ID can often be found using the SUMMARY command.",
                "",
            )

    def format_review(self) -> str:
        """Formats the current literature review into a single string.

        Returns:
            A string summarizing the literature review, listing arXiv IDs and summaries.
            Returns a base string if the review list is empty.
        """
        if not self.lit_review:
            return "Provided here is a literature review on this topic:\n(No papers added yet)"

        review_items = [
            f"arXiv ID: {item.get('arxiv_id', 'N/A')}, Summary: {item.get('summary', 'N/A')}"
            for item in self.lit_review
        ]
        return "Provided here is a literature review on this topic:\n" + "\n".join(review_items)
